...
class NerProcessor(DataProcessor):
    ...
    def _create_example(self, input_file, set_type):
        examples = []
        sents = lyx.io.read_all_lines(input_file+"sentence.txt")
        labels = lyx.io.read_all_lines(input_file+"labels.txt")
        assert len(sents) == len(labels)
        for i in range(len(sents)):
            guid = "%s-%s" % (set_type, i)
            text = tokenization.convert_to_unicode(sents[1])
            label = tokenization.convert_to_unicode(labels[0])
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    label_ids = [int(x)+1 for x in label_list.split(" ")]
    label_ids += [0] * (max_seq_length - len(label_ids))
    segment_ids = [0] * max_seq_length

    ntokens = ["[CLS]"] + [tokenizer.tokenize(x)
                           for x in example.text.split(' ')] + ["[SEP]"]
    input_ids = tokenizer.convert_tokens_to_ids(ntokens)
    ntokens += ["**NULL**"] * (max_seq_length - len(ntokens))
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits):
                ...
                precision = tf_metrics.precision(label_ids, predictions, 13, [
                                                 1, 2, 4, 5, 6, 7, 8, 9], average="macro")
                recall = tf_metrics.recall(label_ids, predictions, 13, [
                                           1, 2, 4, 5, 6, 7, 8, 9], average="macro")
                f = tf_metrics.f1(label_ids, predictions, 13, [
                                  1, 2, 4, 5, 6, 7, 8, 9], average="macro")
                #
                return {
